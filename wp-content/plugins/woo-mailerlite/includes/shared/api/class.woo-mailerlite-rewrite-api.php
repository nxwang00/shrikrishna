...
class RewriteAPI
{
    ...
    public function searchSubscriber($query)
    {
        ...
        // TODO - implement search subscriber
    }
     ...
     * @since       1.6.0
     */
    public function setConsumerData($consumerKey, $consumerSecret, $store, $currency, $group_id, $resubscribe, $ignoreList, $create_segments, $shop_name, $shop_id, $popups_enabled)
    {
     ...
     * @since       1.6.0
     */
    public function createShop($name, $url, $currency, $group_id, $enable_popups, $access_data)
    {
        $response = $this->client->remote_post('/ecommerce/shops', [
            'name' => $name,
            'url' => $url,
    ...
    public function syncOrder( $shop_id, $order_id, $customer, $cart, $status, $total_price, $created_at)
    {
        ...
        $check_order = $this->fetchOrder($shop_id, $order_id);
        ...
        if ($check_order === false) {
            ...
            if (isset($customer['resource_id'])) {

                $customer_exist = $this->fetchCustomer( $shop_id, $customer['resource_id'] );

                if ($customer_exist !== false) {

                    $this->deleteCustomer( $shop_id, $customer['resource_id']);
                }
            }
            ...
            if (isset($check_order->customer->resource_id)) {

                $this->syncCustomer( $shop_id, $check_order->customer->resource_id, $customer['email'], $customer['subscriber_fields']);
            }

            if (isset($check_order->cart)) {

                $this->updateCart($shop_id, $check_order->cart->id, $check_order->cart->checkout_url, $total_price, $order_id);

                $this->replaceCartItems($shop_id, $order_id, $cart['items']);
            }
    ...
    public function syncProduct( $shop_id, $product_id, $name, $price, $exclude_automation, $url, $image, $categories )
    {
        ...
        $response = $this->client->remote_post('/ecommerce/shops/' . $shop_id . '/products?with_resource_id', $parameters);
    ...
    public function importProducts( $shop_id, $products )
    {
        ...
        $response = $this->client->remote_post('/ecommerce/shops/' . $shop_id . '/products/import?with_resource_id&replace_categories', $products);
    ...
    public function importCategories( $shop_id, $categories )
    {
        ...
        $response = $this->client->remote_post('/ecommerce/shops/' . $shop_id . '/categories/import?with_resource_id', $categories);
