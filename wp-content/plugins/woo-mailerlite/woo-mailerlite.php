...
if( ! class_exists( 'Woo_Mailerlite' ) ) {
    ...
    class Woo_Mailerlite {
        ...
        private function setup_constants() {
            ...
            // Plugin version
            define( 'WOO_MAILERLITE_VER', '1.7.13' );
        ...
        private function includes() {
            ...
            require_once WOO_MAILERLITE_DIR . 'includes/shared/api/class.woo-mailerlite-mailerlite-api.php';
            require_once WOO_MAILERLITE_DIR . 'includes/shared/api/class.woo-mailerlite-rewrite-api.php';
            ...
            if ( is_admin() ) {
                require_once WOO_MAILERLITE_DIR . 'includes/admin/functions.php';
        ...
        public function plugin_action_links( $links, $file ) {
            ...
            if ( class_exists( 'WC_Integration' ) ) {
                $settings_link = '<a href="' . admin_url( 'admin.php?page=wc-settings&tab=integration&section=mailerlite' ) . '">' . esc_html__( 'Settings', 'woo-mailerlite' ) . '</a>';
        ...
        private function check_plugin_version() {
            ...
            if ( $transient && $transient->response && is_array( $transient->response ) ) {

                foreach ($transient->response as $plugin) {

                    if ( $plugin->slug === "woo-mailerlite") {
                ...
                if ( $new_version !== false ) {
                    ...
                    if( version_compare(WOO_MAILERLITE_VER, $new_version, '<' ) ) {

                        $warning_msg = __( 'Running older plugin versions may cause problems. Please update ' . WOO_MAILERLITE_NAME . ' plugin to the latest version.', 'woo-mailerlite' );
